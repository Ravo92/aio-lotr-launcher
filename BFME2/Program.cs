    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                {
                    LogHelper.LoggerBFME2GUI.Information(string.Format("Launched after LauncherUpdate now with version: > {0} <", Assembly.GetEntryAssembly()!.GetName().Version));
                    Settings.Default.OpenLauncherChangelogPageAfterUpdate = true;
                    Settings.Default.Save();
                }
                else if (args[0] != "--official")
                {
                    LogHelper.LoggerBFME2GUI.Warning(string.Format("Parameter > {0} < not the expected one at void Main() args in {1}", args[0], assemblyName));
                    return;
                }
            }
            catch (Exception ex)
